...
public abstract class UserMgtServiceAbstractTestCase extends ISIntegrationTest {
    ...
    public void testGetAllRoleNames() throws Exception {
        Assert.assertTrue(nameExists(userMgtClient.listRoles("umRole1", 100), "umRole1"), "Getting all user role " +
                "names has " +
                "failed.");
    }
    ...
    public void testAddRole() throws Exception {
        ...
        Assert.assertTrue(nameExists(userMgtClient.listRoles("umRole2", 100), "umRole2"), "Adding a user role has " +
                "failed");
    }
    ...
    public void testDeleteRole() throws Exception {
        ...
        Assert.assertFalse(nameExists(userMgtClient.listRoles("umRole2", 100), "umRole2"), "Deleting the added user " +
                "role " +
                "has failed");
...
//    	Test add internal role without user
        ...
        allRoles = userMgtClient.listRoles("manager", 0);
        Assert.assertFalse(nameExists(allRoles, "Internal/manager"), "The internal role without user delete has " +
                "failed");
...
//    	Test add internal role with user
    ...
    public void testAddRemoveUsersOfRole() throws Exception {
        ...
        Assert.assertTrue(nameExists(userMgtClient.getUsersOfRole("admin", "user1", 0), "user1"), "Getting user added" +
                " to admin role failed");
        Assert.assertFalse(nameExists(userMgtClient.getUsersOfRole("admin", "user2", 0), "user2"), "User user2 still " +
                "exists in the admin role");

//    	Clean up the modified users of role and test it.
        userMgtClient.updateUserListOfRole("admin", null, newUsers);
        Assert.assertFalse(nameExists(userMgtClient.getUsersOfRole("admin", "user1", 0), "user1"), "User user1 still " +
                "exists in the admin role");
    }
    ...
    public void testAddRemoveRolesOfUser() throws Exception {
        ...
        Assert.assertTrue(nameExists(userMgtClient.getRolesOfUser("user1", "umRole1", 0), "umRole1"), "Adding role to" +
                " user1 has failed");
        Assert.assertFalse(nameExists(userMgtClient.getRolesOfUser("user1", "umRole3", 0), "umRole3"), "Role still " +
                "exists in the user1 roles");

//    	Clean up the modified roles of user and test it.
        userMgtClient.addRemoveRolesOfUser("user1", null, newRoles);
        Assert.assertFalse(nameExists(userMgtClient.getRolesOfUser("user1", "umRole1", 0), "umRole1"), "Role still " +
                "exists in the user1 roles");
    }
    ...
    public void testUpdateUsersOfRole() throws Exception {
        ...
        Assert.assertTrue(nameExists(userMgtClient.getUsersOfRole("umRole1", "user1", 0), "user1"), "Adding user1 to " +
                "role has failed");
        Assert.assertTrue(nameExists(userMgtClient.getUsersOfRole("umRole1", "user2", 0), "user2"), "Adding user2 to " +
                "role has failed");
...
//    	Calling with same user list should delete the users.
        ...
        Assert.assertFalse(nameExists(userMgtClient.getUsersOfRole("umRole1", "user1", 0), "user1"), "Deleting user1 " +
                "from role has failed");
        Assert.assertFalse(nameExists(userMgtClient.getUsersOfRole("umRole1", "user2", 0), "user2"), "Deleting user2 " +
                "from role has failed");
    }
    ...
    public void testUpdateRoleName() throws Exception {
        ...
        Assert.assertFalse(nameExists(userMgtClient.getAllRolesNames("umRole7", 10), "umRole7"), "Role umRole7 update" +
                " failed.");
        Assert.assertTrue(nameExists(userMgtClient.getAllRolesNames("umRole7_1", 10), "umRole7_1"), "Updating role " +
                "umRole7 to umRole7_1 has failed.");
    ...
    public void testGetRolesOfCurrentUser() throws Exception {
        ...
        Assert.assertTrue(nameExists(userMgtClient.getRolesOfCurrentUser(), "umRole5"), "Getting current user roles " +
                "has failed.");
