...
//		}
    ...
    public void testGetAllPolicyIds() throws Exception {
        Assert.assertTrue(Arrays.asList(entitlementPolicyClient.getAllPolicyIds("*")).contains(POLICY_1_ID), "Getting" +
                " all policies has failed.");
    }
    ...
    public void testGetPolicyVersions() throws Exception {
        Assert.assertTrue(Arrays.asList(entitlementPolicyClient.getPolicyVersions(POLICY_1_ID)).contains
                (POLICY_1_VERSION), "Getting policy versions has failed.");
    }
    ...
    @Test(groups = "wso2.is", description = "Check Getting status data", dependsOnMethods =
            "testGetAllPoliciesWithParam")
...
//    }
    ...
    @Test(groups = "wso2.is", description = "Check import policy from registry", dependsOnMethods =
            "testDeleteSubscriber", expectedExceptions = RemoteException.class)
...
//		TODO - need to modify this to have success import without the exception below.
    ...
    @Test(groups = "wso2.is", description = "Check remove policy by id", dependsOnMethods =
            "testImportPolicyFromRegistry")
    public void testRemovePolicy() throws Exception {
        entitlementPolicyClient.removePolicy(POLICY_1_ID, false);
//		TODO - Null pointer when no policies exists by getAllPolicyIds.
//		Assert.assertNotNull(entitlementPolicyClient.getAllPolicyIds("*"), "Possible null pointer exception. Instead
// return empty array");
//		Assert.assertFalse(Arrays.asList(entitlementPolicyClient.getAllPolicyIds("*")).contains(POLICY_1_ID), "Remove
// policy after adding has failed.");
    }
